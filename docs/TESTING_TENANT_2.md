# Tenant-2 End-to-End Pipeline Test (GCP / GAR / GKE / us-central1)

> Step-by-step guide to test the full CI/CD pipeline for **tenant-2** from feature branch
...
| Cloud | GCP |
| Region | `us-central1` |
| Registry | GAR (`us-central1-docker.pkg.dev/<PROJECT>/trunk`) |
| Cluster type | GKE (Standard zonal) |
| Dev cluster | `trunk-dev` (zone: `us-central1-a`) |
| Sandbox cluster | `trunk-sandbox` (zone: `us-central1-a`) |
| Production cluster | `trunk-prod` (zone: `us-central1-a`) |
| Dev namespace | `trunk-dev-tenant-2` |
| Sandbox namespace | `trunk-sandbox-tenant-2` |
| Production namespace | `trunk-production-tenant-2` |
| Currency | EUR |
...
- `GCP_WORKLOAD_IDENTITY_PROVIDER_TENANT_2` — WIF provider (e.g. `projects/<NUM>/locations/global/workloadIdentityPools/github-pool/providers/github-provider`)
- `GCP_SERVICE_ACCOUNT_EMAIL_TENANT_2` — SA email (e.g. `github-actions@<PROJECT>.iam.gserviceaccount.com`)
- `GAR_REGISTRY_TENANT_2` — GAR registry URL (e.g. `us-central1-docker.pkg.dev/<PROJECT>/trunk`)
...
**Goal**: Mirror the hardened base image to tenant-2's GAR.

1. Go to **Actions → DHI Base Image Sync → Run workflow**.
2. Wait for the `sync-tenant-2` job to succeed.
3. Verify:
   ```bash
...
**Stop if** `sync-tenant-2` fails — check `GCP_WORKLOAD_IDENTITY_PROVIDER_TENANT_2`, `GCP_SERVICE_ACCOUNT_EMAIL_TENANT_2`, `GAR_REGISTRY_TENANT_2`, and `DHI_REGISTRY_TOKEN`.
...
1. Create a feature branch:
   ```bash
   git checkout -b feat/tenant-2-test-pipeline
   ```
2. Make a small code change (e.g. add a comment in `services/orders/create.ts`).
3. Add a changeset:
   ```bash
   bun changeset
   # Choose: patch
   # Summary: test tenant-2 pipeline
   ```
4. Commit and push:
   ```bash
   git add . && git commit -m "feat: test tenant-2 pipeline" && git push origin feat/tenant-2-test-pipeline
   ```
...
**Goal**: Merge the PR and verify tenant-2's dev environment receives the deployment.
...
3. Watch **Actions → Deploy to Dev** start automatically.
   - The `tenant-2` matrix job builds 4 images (`trunk-api`, `trunk-orders`, `trunk-notifications`, `trunk-payments`).
   - Images are pushed to GAR with SHA tags.
   - The workflow applies `k8s/overlays/tenant-2/dev`.
4. Verify images were pushed:
   ```bash
   gcloud config configurations activate tenant-2
   gcloud artifacts docker images list us-central1-docker.pkg.dev/<PROJECT>/trunk/trunk-api-dev-tenant-2
   ```
5. Verify pods are running:
   ```bash
   gcloud container clusters get-credentials trunk-dev --zone=us-central1-a
   kubectl get pods -n trunk-dev-tenant-2
   ```
   **(k9s)** `:` → `ctx` → GKE trunk-dev (us-central1-a) context, `:` → `ns` → `trunk-dev-tenant-2`, `:` → `pod` — all 4 service pods should be **Running**.
6. Verify smoke test passed in the workflow logs. Manually port-forward to verify:
   ```bash
   kubectl port-forward deployment/trunk-api 4000:4000 -n trunk-dev-tenant-2
   curl http://localhost:4000/health
   ```
   **(k9s)** `:` → `deploy`, select `trunk-api`, `Shift-F`, set local:4000 → container:4000, then `curl`.
7. Verify ConfigMap values:
   ```bash
   kubectl get configmap trunk-config -n trunk-dev-tenant-2 -o yaml
   ```
   Expect: `TENANT=tenant-2`, `CLOUD=gcp`, `REGION=us-central1`, `NODE_ENV=development`.

**Checkpoint**: tenant-2 dev running with SHA-tagged images. All 4 services healthy.
...
**Goal**: Deploy tenant-2 to sandbox. Verify RC tags and Trivy scan.
...
2. Watch the workflow:
   ...
   - Overlay `k8s/overlays/tenant-2/sandbox` is applied (2 replicas, PDB `minAvailable: 1`).
3. Verify images:
   ```bash
   gcloud config configurations activate tenant-2
   gcloud artifacts docker images list us-central1-docker.pkg.dev/<PROJECT>/trunk/trunk-api-sandbox-tenant-2
   # Expect tags: <sha>, v0.1.1-rc.<N>
   ```
4. Verify pods:
   ```bash
   gcloud container clusters get-credentials trunk-sandbox --zone=us-central1-a
   kubectl get pods -n trunk-sandbox-tenant-2
   ```
   **(k9s)** `:` → `ctx` → GKE trunk-sandbox, `:` → `ns` → `trunk-sandbox-tenant-2`, `:` → `pod`.
   Expect **2 replicas** per service (8 pods total), all Running.
5. Verify PDB:
   ```bash
   kubectl get pdb -n trunk-sandbox-tenant-2
   ```
   Expect `minAvailable: 1` for each service.
6. Manually verify services via port-forward:
   ```bash
   kubectl port-forward deployment/trunk-api 4000:4000 -n trunk-sandbox-tenant-2
   curl http://localhost:4000/health
   ```

**Checkpoint**: tenant-2 sandbox running `v0.1.1-rc.<N>` images. 2 replicas per service. Trivy passed.

---

## Step 7: Deploy to Production

**Goal**: Deploy tenant-2 to production. Verify stable tags, Trivy (CRITICAL+HIGH), and release creation.
...
1. Go to **Actions → Deploy to Production → Run workflow**:
   ...
   | `services` | `all` |
   | `run_migrations` | unchecked |
2. The `production-tenant-2` environment will pause for **required reviewer approval** — approve it.
3. Watch the workflow:
   ...
   - Overlay `k8s/overlays/tenant-2/production` is applied (3 replicas, PDB `minAvailable: 2`, topology spread).
   - Rollout wait is **strict** — a stalled rollout fails the job.
   - Smoke test runs against `trunk-api`.
4. Verify images:
   ```bash
   gcloud config configurations activate tenant-2
   gcloud artifacts docker images list us-central1-docker.pkg.dev/<PROJECT>/trunk/trunk-api
   # Expect tags: <sha>, v0.1.1, latest
   ```
5. Verify pods and topology spread:
   ```bash
   gcloud container clusters get-credentials trunk-prod --zone=us-central1-a
   kubectl get pods -n trunk-production-tenant-2 -o wide
   ```
   Expect **3 replicas** per service (12 pods total). The NODE column should show pods distributed across different nodes.
   **(k9s)** `:` → `ctx` → GKE trunk-prod, `:` → `ns` → `trunk-production-tenant-2`, `:` → `pod` — verify NODE spread.
6. Verify PDB:
   ```bash
   kubectl get pdb -n trunk-production-tenant-2
   ```
   Expect `minAvailable: 2` for each service.
7. After all deploys succeed, `publish-release` runs:
   - Git tag `v0.1.1` is created and pushed (idempotent — skipped if already exists from tenant-1).
...
**Checkpoint**: tenant-2 production running `v0.1.1`. 3 replicas, topology spread active. Git tag and GitHub Release present.

---

## Step 8: Test Rollback

**Goal**: Roll back one service in tenant-2 production and verify isolation.
...
2. Watch the workflow run `kubectl rollout undo deployment/trunk-api -n trunk-production-tenant-2`.
3. Verify:
   ```bash
   kubectl rollout history deployment/trunk-api -n trunk-production-tenant-2
...
5. **Verify isolation**: other services (`trunk-orders`, `trunk-notifications`, `trunk-payments`) in tenant-2 production are unaffected:
   ```bash
   kubectl get pods -n trunk-production-tenant-2
   ```
   Only `trunk-api` pods should have restarted.

**Checkpoint**: `trunk-api` rolled back in tenant-2 production. Other services unaffected.

---

## Step 9: Second Feature PR + Version Bump (Differential Deploy)

**Goal**: Prove tenant-2 can advance to a new version independently.

1. Create another feature branch:
   ```bash
   git checkout main && git pull
   git checkout -b feat/tenant-2-second-feature
   ```
2. Make a small change, add a **minor** changeset:
   ```bash
   bun changeset
   # Choose: minor
   # Summary: test tenant-2 differential deploy
   ```
...
6. Deploy **only tenant-2** through sandbox and production:
...
8. Verify tenant-2 is on the new version:
   ```bash
   kubectl describe deployment trunk-api -n trunk-production-tenant-2 | grep Image
   # → ...trunk-api:v0.2.0
   ```

**Checkpoint**: tenant-2 is on `v0.2.0`. Other tenants remain on their previous versions.

---

## Verification Summary

After completing all steps, confirm for tenant-2:
...
- [ ] Differential deploy: tenant-2 advanced independently to `v0.2.0`
- [ ] ConfigMap `trunk-config` in each namespace has `TENANT=tenant-2`, `CLOUD=gcp`, `REGION=us-central1`
...
gcloud artifacts repositories delete trunk --location=us-central1 --quiet
